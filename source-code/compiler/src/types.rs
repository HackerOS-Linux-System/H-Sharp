...
impl HTypeExt for HType {
    fn size(&self, type_map: &HashMap<String, StructOrUnion>) -> u32 {
        match self {
            ...
            HType::Struct(name, _) | HType::Union(name) => type_map.get(name)
            .map(|s| s.calculate_size(type_map))
            .unwrap_or(0),
            HType::Generic(g) => panic!("Generic type '{}' should have been monomorphized before size calculation! Compiler Error.", g),
        }
    }
    ...
    fn alignment(&self, type_map: &HashMap<String, StructOrUnion>) -> u32 {
        match self {
            ...
            .unwrap_or(1),
            HType::Generic(g) => panic!("Generic type '{}' should have been monomorphized before alignment calculation! Compiler Error.", g),
        }
    }

    fn cl_type(&self) -> cl_types::Type {
        match self {
            HType::I8 | HType::Bool => cl_types::I8,
            HType::I32 => cl_types::I32,
            HType::I64 => cl_types::I64,
