...
/// Generate HTML documentation from `///` doc comments in H# source files.
pub fn generate_docs(extra: &[String], verbose: bool) -> anyhow::Result<()> {
    println!();
    ...
    for file in &files {
        ...
        let items = extract_doc_items(&src, file);
        if verbose {
            println!("  {} {} ({} items)", "·".dimmed(), file, items.len());
        }
    ...
    let mut modules: std::collections::HashMap<String, Vec<&DocItem>> = Default::default();
...
fn collect_hsharp_files(extra: &[String]) -> Vec<String> {
    // Look in src/ and std/ by default
    let roots = ["src", "std", "."];
...
/// Extract all documented items from H# source
fn extract_doc_items(src: &str, file: &str) -> Vec<DocItem> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("///") {
            ...
            // Next non-blank line should be the item
            if i < lines.len() {
...
fn render_index_html(items: &[DocItem]) -> String {
    ...
    .item-kind {{ font-size: 0.75rem; color: #79c0ff; text-transform: uppercase; letter-spacing: 1px; }}
    .item-name {{ font-size: 1.2rem; font-weight: bold; color: #f0f6fc; margin: 4px 0; }}
    .item-sig {{ font-family: monospace; color: #d2a8ff; font-size: 0.9rem; background: #0d1117; padding: 8px 12px; border-radius: 4px; margin: 8px 0; }}
    .item-doc {{ color: #c9d1d9; line-height: 1.6; white-space: pre-wrap; }}
    .module-link {{ color: #58a6ff; text-decoration: none; }}
    ...
    <body>
    <div class="header">
    <h1>⬡ H# Documentation</h1>
    <p>Generated by <code>bytes doc</code> — H# v0.7</p>
    </div>
    <div class="content">
    "#);
    ...
    for (module, mod_items) in &modules {
        let _ = writeln!(
            html,
            "<h2><a class=\"module-link\" href=\"{}.html\">{}</a> ({} items)</h2>",
                         sanitise_filename(module), module, mod_items.len()
        );
...
fn render_module_html(module: &str, items: &[&DocItem]) -> String {
    ...
    .item {{ background: #161b22; border: 1px solid #30363d; border-radius: 8px; margin: 16px 0; padding: 20px; }}
    .item-kind {{ font-size: 0.75rem; color: #79c0ff; text-transform: uppercase; }}
    .item-name {{ font-size: 1.1rem; font-weight: bold; color: #f0f6fc; }}
    .item-sig {{ font-family: monospace; color: #d2a8ff; font-size: 0.85rem; background: #0d1117; padding: 8px; border-radius: 4px; margin: 6px 0; }}
    .item-doc {{ color: #c9d1d9; line-height: 1.6; white-space: pre-wrap; }}
    a {{ color: #58a6ff; }}
    </style>
    </head>
    <body>
    <div class="header">
    <h1>⬡ H# — {module} <small style="font-size:0.6em; color:#8b949e;">module</small></h1>
    </div>
    <div class="content">
